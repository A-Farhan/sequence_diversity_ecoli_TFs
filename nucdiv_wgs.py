# Calculate nucleotide diversity from WGS data
...
def wrapper(gene,runs=runs,gene_vars=gene_vars,run_genes=run_genes):
    ...
    # compute Nucleotide diversity from the above matrix
    return nucdiv_wgscodonmat(gene,mat)

## apply the wrapper function parallely to all genes with variants
out = Parallel( n_jobs = nt) ( map( delayed(wrapper), genes))
out = filter(None,out)
writecsv(data=out,fl=f_nucdiv,sep='\t',header=['gene','runs','sites','nucdiv'])
